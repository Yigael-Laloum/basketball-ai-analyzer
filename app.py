...
st.markdown(
    ...
    /* יישור כותרות, טקסט רגיל ותוויות */
    h1, h2, h3, p, span, label, .stMarkdown {
        text-align: right !important;
        direction: rtl !important;
    }

    /* יישור תיבות בחירה (Radio Buttons) */
    div[role="radiogroup"] {
        direction: rtl;
        display: flex;
        gap: 20px;
...
def analyze_basketball_clip(video_path: str):
    ...
    try:
        ...
        with st.spinner("מעלה וידאו ל-AI ומנתח..."):
            ...
            prompt = "נתח את אירוע השיפוט בסרטון הכדורסל הזה לפי חוקת FIBA. התייחס למגע, מיקום השופט וההחלטה. ענה בעברית."
...
if source == "YouTube URL":
    ...
    # שים לב: שיניתי ל-file_uploader כי file_upload לא קיים בגרסאות סטנדרטיות
    uploaded_file = st.file_uploader("בחר קובץ וידאו", type=['mp4', 'mov', 'avi'])
    if uploaded_file:
        temp_dir = tempfile.gettempdir()
